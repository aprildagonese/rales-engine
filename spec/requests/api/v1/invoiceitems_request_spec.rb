...
RSpec.describe "InvoiceItems API" do
  ...
  describe "Finders" do
    it "can find a single object based on parameters" do
      ...
      @invoice = create(:invoice, customer: @customer, merchant: @merchant)
      @item1, @item2, @item3 = create_list(:item, 3, merchant: @merchant)
      @invoiceitem1 = create(:invoice_item, invoice: @invoice, item: @item1, quantity: 1, unit_price: 100, created_at: "2019-03-06 21:29:54 UTC", updated_at: "2019-03-07 21:29:54 UTC")
      @invoiceitem2 = create(:invoice_item, invoice: @invoice, item: @item2, quantity: 2, unit_price: 200, created_at: "2019-03-08 21:29:54 UTC", updated_at: "2019-03-09 21:29:54 UTC")
      @invoiceitem3 = create(:invoice_item, invoice: @invoice, item: @item3, quantity: 3, unit_price: 300, created_at: "2019-03-10 21:29:54 UTC", updated_at: "2019-03-11 21:29:54 UTC")


      get "/api/v1/invoice_items/find?id=#{@invoiceitem3.id}"
      expect(response).to be_successful
      invoiceitem = JSON.parse(response.body)
      ...
      get "/api/v1/invoice_items/find?quantity=#{@invoiceitem3.quantity}"
      expect(response).to be_successful
      invoiceitem = JSON.parse(response.body)
      expect(invoiceitem["data"]["attributes"]["id"]).to eq(@invoiceitem3.id)
      ...
      get "/api/v1/invoice_items/find?created_at=#{@invoiceitem1.created_at}"
      expect(response).to be_successful
      invoiceitem = JSON.parse(response.body)
      expect(invoiceitem["data"]["attributes"]["id"]).to eq(@invoiceitem1.id)

      get "/api/v1/invoice_items/find?updated_at=#{@invoiceitem2.updated_at}"
      expect(response).to be_successful
      invoiceitem = JSON.parse(response.body)
      expect(invoiceitem["data"]["attributes"]["id"]).to eq(@invoiceitem2.id)
    end
    ...
    it "can find_all objects based on case-insensitive parameters" do
      ...
      @invoice = create(:invoice, customer: @customer, merchant: @merchant)
      @item1, @item2, @item3 = create_list(:item, 3, merchant: @merchant)
      @invoiceitem1 = create(:invoice_item, invoice: @invoice, item: @item1, quantity: 1, unit_price: 100, created_at: "2019-03-08 21:29:54 UTC", updated_at: "2019-03-07 21:29:54 UTC")
      @invoiceitem2 = create(:invoice_item, invoice: @invoice, item: @item2, quantity: 1, unit_price: 300, created_at: "2019-03-08 21:29:54 UTC", updated_at: "2019-03-11 21:29:54 UTC")
      @invoiceitem3 = create(:invoice_item, invoice: @invoice, item: @item2, quantity: 3, unit_price: 300, created_at: "2019-03-10 21:29:54 UTC", updated_at: "2019-03-11 21:29:54 UTC")

      get "/api/v1/invoice_items/find_all?id=#{@invoiceitem3.id}"
      expect(response).to be_successful
      invoiceitems = JSON.parse(response.body)
      ...
      get "/api/v1/invoice_items/find_all?quantity=#{@invoiceitem1.quantity}"
      expect(response).to be_successful
      invoiceitems = JSON.parse(response.body)
      expect(invoiceitems["data"][0]["attributes"]["id"]).to eq(@invoiceitem1.id)
      expect(invoiceitems["data"][1]["attributes"]["id"]).to eq(@invoiceitem2.id)
      ...
      get "/api/v1/invoice_items/find_all?created_at=#{@invoiceitem1.created_at}"
      expect(response).to be_successful
      invoiceitems = JSON.parse(response.body)
      expect(invoiceitems["data"][0]["attributes"]["id"]).to eq(@invoiceitem1.id)
      expect(invoiceitems["data"][1]["attributes"]["id"]).to eq(@invoiceitem2.id)

      get "/api/v1/invoice_items/find_all?updated_at=#{@invoiceitem2.updated_at}"
      expect(response).to be_successful
      invoiceitems = JSON.parse(response.body)
      expect(invoiceitems["data"][0]["attributes"]["id"]).to eq(@invoiceitem2.id)
      expect(invoiceitems["data"][1]["attributes"]["id"]).to eq(@invoiceitem3.id)
  ...
  describe "Relationship Endpoints" do
    ...
    it "returns associated invoice" do
      get "/api/v1/invoice_items/#{@invoiceitem1.id}/invoice"
      expect(response).to be_successful
    ...
    it "returns associated invoice" do
      get "/api/v1/invoice_items/#{@invoiceitem2.id}/item"
      expect(response).to be_successful
